...
PROPOSALS="$WS/OpenClawData/revenue/proposals"
FOLLOWUPS="$WS/OpenClawData/revenue/followups"
PIPELINE="$WS/OpenClawData/revenue/pipeline-state"
...
case "$ACTION" in
  --status)
    ...
    HOT=$(grep -l '"hot"' "$LEADS"/*.json 2>/dev/null | wc -l | tr -d ' ')
    WARM=$(grep -l '"warm"' "$LEADS"/*.json 2>/dev/null | wc -l | tr -d ' ')
