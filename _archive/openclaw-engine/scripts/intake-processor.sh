...
# Reads: data/source-notes/, source-links/, product-updates/, screenshots/
# Writes: Adds YAML frontmatter classification to files, copies to appropriate staging folders
# Logs: logs/intake-processor.log
...
LOG_FILE="$WORKSPACE_ROOT/logs/intake-processor.log"
...
SOURCE_DIRS=(
    "$WORKSPACE_ROOT/data/source-notes"
    "$WORKSPACE_ROOT/data/source-links"
    "$WORKSPACE_ROOT/data/product-updates"
)

PROCESSED_LOG="$WORKSPACE_ROOT/logs/intake-processed-files.log"
touch "$PROCESSED_LOG"
...
SUMMARY_FILE="$WORKSPACE_ROOT/logs/intake-summary-$DATE_TAG.log"
